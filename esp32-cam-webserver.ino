...
void WifiSetup() {
    ...
    Serial.printf("Connecting to Wifi Network %s", stationList[0].ssid);

    WiFi.setHostname(mdnsName);

    // Initiate network connection request (3rd argument, channel = 0 is 'auto')
    WiFi.begin(stationList[0].ssid, stationList[0].password);
...
void setup() {
    ...
    if (filesystem) {
        filesystemStart();
        delay(50); // a short delay to let spi bus settle after SPIFFS init
    }
    ...
    if (filesystem) {
        delay(50); // a short delay to let spi bus settle after camera init
        loadPrefs(SPIFFS);
    } else {
